...
const parseImageSources = root => {
    if (!root) return DEFAULT_IMAGES;
    if (Array.isArray(window.DESIGN_IMAGES) && window.DESIGN_IMAGES.length > 0) {
        return window.DESIGN_IMAGES;
    }
    const datasetImages = root.dataset.images;
    if (!datasetImages) return DEFAULT_IMAGES;
    try {
        const parsed = JSON.parse(datasetImages);
        return Array.isArray(parsed) && parsed.length > 0 ? parsed : DEFAULT_IMAGES;
    } catch (err) {
        console.warn('[Design Gallery] Failed to parse data-images JSON:', err);
        return DEFAULT_IMAGES;
    }
};

function DesignPortfolioApp() {
    const rootEl = document.getElementById('design-root');
    const [images, setImages] = useState(() => parseImageSources(rootEl));
    ...
    useEffect(() => {
        const observer = new MutationObserver(() => {
            setImages(parseImageSources(rootEl));
        });
        if (rootEl) {
            observer.observe(rootEl, { attributes: true, attributeFilter: ['data-images'] });
        }
    ...
    useEffect(() => {
        window.setDesignGalleryImages = newImages => {
            if (!Array.isArray(newImages) || newImages.length === 0) return;
            setImages(newImages);
        };
        return () => {
            delete window.setDesignGalleryImages;
        };
    }, []);
...
if (container) {
    ...
    root.render(<DesignPortfolioApp />);
}